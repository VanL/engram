...
`docs/plans/2026-07-17-agent-guidance-propagation-plan.md`. Not promoted
in Engram: the source repo hit the [DOM-14] promotion tier and promoted
it there (3 distinct citations of this workflow theme: taut MCP plan
§16.3–§16.4; mm `implementation/41` RiskEvaluationApi contract; mm
external-API/MCP contract plan §4/§5/§9), recorded in agent-guidance's
`docs/coalescing.md` run log and its
`docs/plans/2026-07-15-interface-review-skill-promotion-plan.md` (foreign
hub plan, cited by name). The skill content was reviewed at the source;
Engram's landing review is scoped to the adaptation.
...
- `docs/agent-context/engineering-principles.md` §13 (Enumerable
  Contracts Get Executable Gates; this repo's numbering) — the gate
  every enumerable element owes
...
- Enumerate each error-code set, guidance-type set, status enum,
  ...
  engineering-principles §13 (this repo's numbering).
