...
- `docs/agent-context/engineering-principles.md` §4 (read before
  inference), §11 (failing test first — this repo's numbering)
...
2. **Read before inferring** (§4): the spec, the implementation, the
...
5. **Write the failing test, watch it fail** (§11), then fix, then watch
