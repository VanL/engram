...
| superpowers:test-driven-development | `../engineering-principles.md` §11 (this repo's numbering); `testing-patterns.md` Rule 5 | §11 is absolute about understanding (no failing test → you don't understand the problem yet). The named-substitute-proof allowance lives in testing-patterns Rule 5, not §11: when red-green is impractical, say what replaced it and why — never skip silently |
...
| superpowers:requesting-code-review, receiving-code-review | `review-loops-and-agent-bootstrap.md`; [DOM-11] | Reproduce findings before acting (`../engineering-principles.md` §9 — this repo's numbering); record dispositions in the plan |
...
- **File size.** superpowers:writing-plans prefers "smaller, focused
  files." This repo's `engineering-principles.md` §15 (cohesion over file
...
- **TDD absolutism.** superpowers treats TDD as unconditional. This repo's
  §11 keeps the pressure (a missing failing test means missing
...
- External-skill firings are a **hint to check**, not promotion evidence:
  [DOM-14] and `../engineering-principles.md` §16 (this repo's numbering) count only citations in
