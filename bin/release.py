#!/usr/bin/env python3
"""Repo-local release helper for Engram maintainers."""
...
CONSTANTS_PATH: Final[Path] = PROJECT_ROOT / "engram" / "_constants.py"
UV_LOCK_PATH: Final[Path] = PROJECT_ROOT / "uv.lock"
RELEASE_GATE_WORKFLOW: Final[str] = ".github/workflows/release-gate.yml"
PYPI_RELEASE_WORKFLOW: Final[str] = ".github/workflows/release.yml"
...
ROOT_RELEASE_TARGET: Final[ReleaseTarget] = ReleaseTarget(
    ...
    release_gate_workflow=RELEASE_GATE_WORKFLOW,
...
def read_current_version(
    *,
    ...
    constants_version = _extract_version(
        ...
        label="engram/_constants.py",
    )
    if pyproject_version != constants_version:
        raise RuntimeError(
            "Version mismatch between pyproject.toml "
            f"({pyproject_version}) and engram/_constants.py ({constants_version})"
        )
...
def build_postupdate_steps() -> tuple[CommandStep, ...]:
    """Return post-version-update verification and build steps."""
...
def _url_exists(url: str) -> bool:
    ...
    headers = {
        ...
        "User-Agent": "engram-release-helper",
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Deprecated compatibility flag. The tag-push release gate creates "
            "GitHub Release artifacts; direct PyPI publication remains disabled."
        ),
    )
    parser.add_argument(
        "--skip-checks",
...
def _print_publish_note() -> None:
    print(
        "--publish is ignored: "
        f"{RELEASE_GATE_WORKFLOW} creates GitHub Release artifacts after the "
        "tag gate passes; direct PyPI publication remains disabled in "
        f"{PYPI_RELEASE_WORKFLOW}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        if version_changed:
            run_command(
                ("git", "add", "pyproject.toml", "engram/_constants.py", "uv.lock"),
                dry_run=True,
            )
            run_command(
        ...
        print(
            ...
            f"{RELEASE_GATE_WORKFLOW} on {release_state.tag_name}. "
    ...
    if version_changed:
        run_command(("git", "add", "pyproject.toml", "engram/_constants.py", "uv.lock"))
