...
|------|---------|------------|-------|
| 2026-07-28 | — (propagation; nothing folded) | source agent-guidance @ `cec5666` (extraction opened at `51626db` with four fold-up items worktree-only; the hub committed them mid-landing as `e42762c`/`cec5666` and the payload was re-verified against that end-state) | Delta wave per `docs/plans/2026-07-28-agent-guidance-propagation-plan.md`: coalescing skill gains the cue-portability paragraph, the repair-in-sweep doctrine, the structured-status-index derivation clause, and the matching Purpose line; writing-plans gains the closed status-vocabulary bullet (`status-review` quarantine); `AGENTS.md` gains the harness-scoping sentence. Two executable gates adopted and adapted: `bin/check-doc-paths` and `bin/coalesce-check`. Hub lineage for the fold-ups is taut's status-index contract and repair-in-sweep doctrine (taut commit `3706d73`, foreign, quoted by name). Term disambiguation (documentation ledger vs Engram's product memory coalescing) preserved and reinforced at the new repair-in-sweep block. **Gate baseline recorded, not repaired:** `check-doc-paths` reports 6 pre-existing dangling path claims (4 spec `## Related Plans` rows naming hub plans never resident here; 2 backticked foreign-hub-plan citations in propagated guidance) — outside this wave's payload, owner disposition owed. `coalesce-check` exit 0: 8 SHA claims (6 resolved in siblings), 0 cues, lessons dated entries 2; local-only pins `f92fa82`, `b1d799e`. `check-dom15-fixtures` exit 0. Additive-only: no thresholds, watermarks, or folds touched. Landing commit may be appended here as metadata once it exists. |
