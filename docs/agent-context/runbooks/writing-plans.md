...
- **The status vocabulary is closed, and ambiguity has a name.** Valid
  ...
  data is never rewritten to make a checker pass). (Adopted 2026-07-28
  from agent-guidance's delta wave via
  `docs/plans/2026-07-28-agent-guidance-propagation-plan.md`; the hub
  folded it up from taut's status-index contract, taut commit
  `3706d73`, quoted here as foreign lineage. Second lineage recorded
  by the hub: mm's backfill campaign independently invented the same
  quarantine as free text under pressure.)
...
- **Approval attaches to the text that was reviewed, not to the file
  ...
  candidates — tracked in agent-guidance's `docs/coalescing.md`, quoted
  here as foreign lineage.)
