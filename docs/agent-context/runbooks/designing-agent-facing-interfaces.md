...
accounting in agent-guidance's
`docs/plans/2026-07-14-agent-facing-interfaces-runbook-plan.md` §2 (a
foreign hub plan, cited by name, not resident here).

*This copy: adopted from agent-guidance @ `b248e1c` via
`docs/plans/2026-07-17-agent-guidance-propagation-plan.md`. The runbook
content was reviewed at the source; Engram's landing review is scoped to
the adaptation.*
...
7. **Teach, don't reject.** Canonicalize near-miss input at the
   ...
   doctrine (Canonicalize at Boundaries — an engineering principle in
   the agent-guidance hub; Engram carries the concept in prose, not as
   a numbered principle). Distinct from
...
are enumerable contracts governed by engineering-principles §13
(Enumerable Contracts Get Executable Gates; this repo's numbering), and
any agent-facing tool
